...
docker run --name auth --net host -p 3000:3000 -e TESTING=true -d dadebulba/apiatori-auth

# Start gateway
docker build -f microservices/gateway/Dockerfile -t dadebulba/apiatori-gateway .
docker run --name gateway --net host -p 8080:8080 -e TESTING=true -d dadebulba/apiatori-gateway

# Start spaces service
docker build -f microservices/spaces/Dockerfile -t dadebulba/apiatori-spaces .
docker run --name spaces --net host -p 3002:3002 -e TESTING=true -d dadebulba/apiatori-spaces

# Start groups service
docker build -f microservices/groups/Dockerfile -t dadebulba/apiatori-groups .
docker run --name groups --net host -p 3003:3003 -e TESTING=true -d dadebulba/apiatori-groups

# Start users service
docker build -f microservices/users/Dockerfile -t dadebulba/apiatori-users .
docker run --name users --net host -p 3004:3004 -e TESTING=true -d dadebulba/apiatori-users
